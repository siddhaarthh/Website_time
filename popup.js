document.addEventListener('DOMContentLoaded', function() {
  ...
  addDomainButton.addEventListener('click', function() {
      ...
      if (domain) {
          saveRestrictedSite(domain);
          domainInput.value = '';
          refreshRestrictedSitesList();
      }
  });

  function refreshRestrictedSitesList() {
      restrictedSitesList.innerHTML = '';
      getRestrictedSites(function(sites) {
  ...
  function saveRestrictedSite(site) {
  ...
  function removeRestrictedSite(site) {
      chrome.storage.local.get({ restrictedSites: [] }, function(result) {
          const restrictedSites = result.restrictedSites;
          const index = restrictedSites.indexOf(site);
          if (index !== -1) {
  ...
  function getRestrictedSites(callback) {
      chrome.storage.local.get({ restrictedSites: [] }, function(result) {
          callback(result.restrictedSites);
      });
  }
      ...
      if(calendar.value === "") {
          ...
          chrome.storage.local.get(givenDate, function(thatDay) {
              if(thatDay[givenDate] == null) {
                  ...
                  document.getElementById("totalTimeDifferentDay").innerText = secondsToString(thatDayTotal);
                  const table = document.getElementById("differentDayTable");
                  table.innerHTML = "";
                  for(let i = 0; i < times.length; i++){
                    let tableRow = document.createElement("tr");
                    let tableNumber = document.createElement("td");
                    tableNumber.innerText = i + 1;
                    let tableURL = document.createElement("td");
                    tableURL.innerText = times[i][0];
                    let tableTime = document.createElement("td");
                    tableTime.innerText = secondsToString(times[i][1]);
